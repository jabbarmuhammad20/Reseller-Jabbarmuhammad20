										...
										<div class="item form-group{{ $errors->has('kategori') ? ' has-error' : '' }}">
											...
											<div class="col-md-6 col-sm-6 ">
												<select class="form-control" id="kategori_id" name="kategori_id">
													<option value="{{$produk_tabel->kategori->id}}">{{$produk_tabel->kategori->n_kategori}} <i class="fa fa-check text-blue"></i></option>

													@foreach ($kategori as $kategori)
													<option value="{{$kategori->id}}">{{$kategori->id}} {{$kategori->n_kategori}}</option>
													@endforeach
												</select>
											</div>
